...
SIGNALE : Incident déclaré par un citoyen
PRIS_EN_CHARGE : Un agent a pris en charge l'incident
EN_RESOLUTION : L'incident est en cours de traitement
RESOLU : L'incident a été résolu
CLOTURE : L'incident est clôturé par le citoyen
Priorités
BASSE : Impact limité
...
Notifications Internes : Toujours enregistrées en base de données
Email : Envoi d'emails selon les préférences utilisateur
WebSocket : Notifications push en temps réel dans l'interface
...
Sécurité : Ne pas exposer directement les entités JPA (éviter les fuites de données)
Performance : Ne transférer que les données nécessaires
...
CI (Intégration Continue) : Build et tests
CD (Déploiement Continu) : Build Docker et déploiement
...
Vérifier le statut de chaque étape (✅ ou ❌)
...
Projet développé dans le cadre d'un projet académique/professionnel de gestion d'incidents urbains. Mayssa Ben azzouz - Eya Ouni - Sirine Berrbibe
